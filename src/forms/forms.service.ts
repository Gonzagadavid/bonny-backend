...
export class FormsService {
  ...
  async create(createFormDto: CreateFormDto, user: UserPayload) {
    ...
    return this.formModel.create({...createFormDto, active: false, createdAt: new Date(), createdBy: user.userId});
  }

  async activeForm(id: string) {
    const currentActive = await this.formModel.findOne({active: true})
    if(currentActive) [
      await this.formModel.updateOne({_id: currentActive._id}, {active: false})
    ]
